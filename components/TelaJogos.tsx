...
export default function TelaJogos({ statusF, setStatusF, rodada, setRodada, jogosFiltrados, jogosRodada, res, palS, palpitesMap, F, setJogoSel }: Props) {
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
        <button className="nbtn" onClick={() => setRodada(Math.max(1, rodada - 1))} disabled={rodada === 1}>←</button>
        ...
        <button className="nbtn" onClick={() => setRodada(Math.min(3, rodada + 1))} disabled={rodada === 3}>→</button>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        {jogosFiltrados.map(j => {
          ...
          let mT = "";
          if (tR && tP) {
            const { tipo } = calcJogo(parseInt(pJ.gols1), parseInt(pJ.gols2), parseInt(r.gols1), parseInt(r.gols2), j.fase || "grupos", r.penalti || false);
            mT = tipo;
          }
          let nAc = 0, nPal = 0;
          if (tR) {
            Object.values(palpitesMap).forEach((ps: any) => {
              ...
              nPal++;
              const { tipo } = calcJogo(parseInt(p.gols1), parseInt(p.gols2), parseInt(r.gols1), parseInt(r.gols2), j.fase || "grupos", r.penalti || false);
